...
final class KokoroTokenizer {
    ...
    func setLanguage(for voice: TTSVoice) throws {
        ...
        if currentLanguage != language {
            ...
            // Update lexicon based on language
            switch language {
            case .enUS:
                if self.lexicon == nil {
                    setLexicon(filename: "us_gold")
                }
            default:
                self.lexicon = nil
            }
    ...
    /// Set custom lexicon for word-to-phoneme mappings
    func setLexicon(filename: String) {
